                                    ...
                                    intptr_t stride)
{
  ...
  /* Diagnostic (r31): register/capture the gbuffer right after the prepass submit.
   * No-op unless BW_DIAG is set (wgpu_diag_readback). */
  webgpu::diag::on_draw_submit(shader->name_get().c_str(), fb);
}

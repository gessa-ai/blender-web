 ...
 * \ingroup gpu
 *
 * Shader source dependency builder that make possible to support #include <cstdio>
...
struct GPUSource {
      ...
      : fullpath(path), filename(file), source(datatoc)
  {
    if (source.find("//") != std::string::npos || source.find("/*") != std::string::npos) {
      size_t _i = source.find("//"); if (_i == std::string::npos) _i = source.find("/*");
      fprintf(stderr, "[SHADERCOMMENT] %s @%zu :: %.70s\n", file, _i,
              source.c_str() + (_i > 20 ? _i - 20 : 0));
    }

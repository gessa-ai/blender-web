...
| `.m8-browsers/Google Chrome.app`, `Microsoft Edge.app`, signed geckodriver | Not transferred. The current M8 Chrome+Edge matrix and 30-second product producer now verify the canonical Linux Chrome/Edge package ELFs through exact `dpkg`/APT/keyring identities; install instructions and paths are in `sandbox/m8-launch-gate/README.md`. Their live rows still wait on s7/APPLY. Port the remaining M8 performance/soak/staged producers before using them on Linux; the M7 Safari row remains macOS-only. |

...
NATIVE_BUILD="${NATIVE_BUILD:-$ROOT/build-deps/m5-grease-pencil-primitive-depth-cache/native}"
WASM_BUILD="${WASM_BUILD:-$ROOT/build-deps/m5-grease-pencil-primitive-depth-cache/wasm}"
OUT="${OUT:-$ROOT/build-deps/m5-grease-pencil-primitive-depth-cache/evidence}"
PATCH="$ROOT/patches/0269-m5-grease-pencil-primitive-depth-cache-continuation.patch"
CANONICAL="$ROOT/patches/PREVIEW_SNAPSHOT.patch"
CANONICAL_SHA="$ROOT/patches/PREVIEW_SNAPSHOT.sha256"
RELATIVE="source/blender/editors/grease_pencil/intern/grease_pencil_primitive.cc"
OBJECT="source/blender/editors/grease_pencil/CMakeFiles/bf_editor_grease_pencil.dir/intern/grease_pencil_primitive.cc.o"
...
if [ -z "$SOURCE_ROOT" ]; then
  SOURCE_ROOT="$(mktemp -d -t bw-m5-gp-primitive-depth-source.XXXXXX)"
  cleanup_source()
  {
    rm -rf -- "$SOURCE_ROOT"
  }
  ...
  git -C "$ROOT/upstream" archive --format=tar "$PIN" -- "$RELATIVE" | \
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" \
  m5_grease_pencil_primitive_depth_cache_contract
...
"$NATIVE_BUILD/m5_grease_pencil_primitive_depth_cache_contract" \
  >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_grease_pencil_primitive_depth_cache_contract.js" \
...
if ! jq -e '
  .verdict == "PASS" and
  .contracts.native_immediate_initialization == true and
  .contracts.stock_initial_failure_fallback == true and
  .contracts.pending_before_first_projection == true and
  .contracts.bounded_event_fifo == true and
  .contracts.producing_context_guard == true and
  .contracts.cursor_navigation_ownership == true and
  .contracts.exact_once_settlement == true and
  .contracts.failure_timeout_cancellation == true and
  .contracts.live_hardware_receipt == false and
  (.converted_callers | length) == 6 and
  .remaining_grease_pencil_callers == ["fill", "pen-helper"] and
...
for graph in "$NATIVE_GRAPH" "$WASM_GRAPH"; do
  ...
  commands="$OUT/${graph_label}-grease_pencil_primitive.commands"
  object="$OUT/${graph_label}-grease_pencil_primitive.o"
  "$ROOT/scripts/ninja-locked.sh" -C "$graph" -t commands "$OBJECT" >"$commands"
  "$PYBIN" "$COMPILE_OVERLAY" --commands-log "$commands" \
    --relative "$RELATIVE" --source-root "$SOURCE_ROOT" --source "$SOURCE_ROOT/$RELATIVE" \
    --output "$object" >"$OUT/${graph_label}-grease_pencil_primitive.stdout"
done
...
printf 'PASS m5-grease-pencil-primitive-depth-cache native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s canonical_sha256=%s emcc=%s node=v22.16.0 callers=6 gp_followups=2 remaining_sync=2 live_receipt=false\n' \

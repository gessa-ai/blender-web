...
"""Reverse and forward patch 0265 against its exact current postimages."""
...
EXPECTED_NAME = "0265-m5-curve-depth-cache-continuation.patch"
EXPECTED_PATHS = {
    Path("source/blender/editors/include/ED_view3d.hh"),
    Path("source/blender/editors/space_view3d/view3d_draw.cc"),
    Path("source/blender/editors/curve/editcurve_paint.cc"),
    Path("source/blender/editors/curves/intern/curves_draw.cc"),
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="m5-curve-depth-cache-patch-") as temporary:
    ...
    print(f"M5_CURVE_DEPTH_CACHE_NUMBERED_PATCH_PASS files=4 sha256={digest}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"M5_CURVE_DEPTH_CACHE_NUMBERED_PATCH_FAIL {error}", file=__import__("sys").stderr)
        raise SystemExit(1)

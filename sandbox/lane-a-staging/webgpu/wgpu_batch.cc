...
void WGPUBatch::draw(int vertex_first, int vertex_count, int instance_first, int instance_count)
{
  ...
  /* Bind group 0: the shader's push-constant UBO (lane A's shadow+flush+getter).
   * uniform_float/int fill the CPU shadow; flush uploads the dirty bytes, then the
   * buffer binds at its reserved binding. Textures / extra UBOs (state_manager
   * bindings) are added here once the texture bind path lands (follow-on). */
  if (shader->has_push_constants()) {
    shader->push_constants_flush();
...
{
  /* Indirect draw (DrawIndirect / DrawIndexedIndirect) — lands with the indirect
   * buffer draw path; not exercised by the current gate. */
  BLI_assert_msg(0, "WGPUBatch::draw_indirect not implemented yet");
}
...
void WGPUBatch::multi_draw_indirect(StorageBuf * /*indirect_buf*/,
                                    ...
                                    intptr_t /*stride*/)
{
  BLI_assert_msg(0, "WGPUBatch::multi_draw_indirect not implemented yet");
}

...
#
# r44r2 workbench 20-scene re-score against the m6 goldens on the namespace-fixed build.
# Boots each .blend as the startup file (own bridge_boot.mjs -> gpu-r44-r2 caps, port 8128),
# pulls the render-result via the BW_DIAG bridge (0125), scores with the r35 score.py at the
# pinned per-scene thresholds. Writes sandbox/gpu-r44-r2/results.tsv (does NOT touch r35's).
set -uo pipefail
cd "$(git rev-parse --show-toplevel)"
ROOT="$(pwd)"
R35="$ROOT/sandbox/gpu-r35"
R2="$ROOT/sandbox/gpu-r44-r2"

...
RELATIVE_HEADER="source/blender/editors/include/ED_particle.hh"
RELATIVE_PARTICLE="source/blender/editors/physics/particle_edit.cc"
RELATIVE_VIEW3D="source/blender/editors/space_view3d/view3d_select.cc"
PARTICLE_OBJECT="source/blender/editors/physics/CMakeFiles/bf_editor_physics.dir/particle_edit.cc.o"
VIEW3D_OBJECT="source/blender/editors/space_view3d/CMakeFiles/bf_editor_space_view3d.dir/view3d_select.cc.o"
...
if [ -z "$SOURCE_ROOT" ]; then
  SOURCE_ROOT="$(mktemp -d -t bw-m5-particle-depth-source.XXXXXX)"
  cleanup_source()
  {
    rm -rf -- "$SOURCE_ROOT"
  }
  ...
  git -C "$ROOT/upstream" archive --format=tar "$PIN" -- \
    "$RELATIVE_HEADER" "$RELATIVE_PARTICLE" "$RELATIVE_VIEW3D" | tar -xf - -C "$SOURCE_ROOT"
  git -C "$SOURCE_ROOT" apply --check \
    --include="$RELATIVE_HEADER" --include="$RELATIVE_PARTICLE" --include="$RELATIVE_VIEW3D" \
    "$CANONICAL"
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" m5_particle_edit_depth_cache_contract
...
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" m5_particle_edit_depth_cache_contract

NATIVE_STDOUT="$OUT/native.stdout"
NATIVE_STDERR="$OUT/native.stderr"
WASM_STDOUT="$OUT/wasm.stdout"
WASM_STDERR="$OUT/wasm.stderr"
"$NATIVE_BUILD/m5_particle_edit_depth_cache_contract" >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_particle_edit_depth_cache_contract.js" \
...
if ! jq -e '
  .verdict == "PASS" and
  .contracts.owned_prepare_consume == true and
  .contracts.xray_native_immediate == true and
  .contracts.pre_traversal_suspension == true and
  .contracts.exact_operator_inputs == true and
  .contracts.one_shot_gesture_owners == true and
...
for graph in "$NATIVE_GRAPH" "$WASM_GRAPH"; do
  ...
  particle_commands="$OUT/${graph_label}-particle_edit.commands"
  view3d_commands="$OUT/${graph_label}-view3d_select.commands"
  "$ROOT/scripts/ninja-locked.sh" -C "$graph" -t commands "$PARTICLE_OBJECT" \
    >"$particle_commands"
  "$ROOT/scripts/ninja-locked.sh" -C "$graph" -t commands "$VIEW3D_OBJECT" \
    >"$view3d_commands"
  "$PYBIN" "$COMPILE_OVERLAY" --commands-log "$particle_commands" \
    --relative "$RELATIVE_PARTICLE" --source-root "$SOURCE_ROOT" \
    --source "$SOURCE_ROOT/$RELATIVE_PARTICLE" \
    --output "$OUT/${graph_label}-particle_edit.o" \
    >"$OUT/${graph_label}-particle_edit.stdout"
  "$PYBIN" "$COMPILE_OVERLAY" --commands-log "$view3d_commands" \
    --relative "$RELATIVE_VIEW3D" --source-root "$SOURCE_ROOT" \
    --source "$SOURCE_ROOT/$RELATIVE_VIEW3D" \
    --output "$OUT/${graph_label}-view3d_select.o" \
    >"$OUT/${graph_label}-view3d_select.stdout"
done

"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" -n m5_particle_edit_depth_cache_contract
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" -n m5_particle_edit_depth_cache_contract
...
printf 'PASS m5-particle-edit-depth-cache native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s canonical_sha256=%s emcc=%s node=v22.16.0 callers=6 remaining_depth=0 remaining_sync=1 live_receipt=false\n' \

...
RELATIVE="source/blender/editors/object/object_transform.cc"
OBJECT="source/blender/editors/object/CMakeFiles/bf_editor_object.dir/object_transform.cc.o"
...
if [ -z "$SOURCE_ROOT" ]; then
  SOURCE_ROOT="$(mktemp -d -t bw-m5-object-axis-target-depth-source.XXXXXX)"
  cleanup_source()
  {
    rm -rf -- "$SOURCE_ROOT"
  }
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" \
  m5_object_axis_target_depth_cache_contract
...
"$NATIVE_BUILD/m5_object_axis_target_depth_cache_contract" \
  >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_object_axis_target_depth_cache_contract.js" \
...
if ! jq -e '
  ...
  .contracts.render_override_restoration == true and
  .contracts.stock_no_depth_cancellation == true and
...
for graph in "$NATIVE_GRAPH" "$WASM_GRAPH"; do
  ...
  commands="$OUT/${graph_label}-object_transform.commands"
  output_object="$OUT/${graph_label}-object_transform.o"
  "$ROOT/scripts/ninja-locked.sh" -C "$graph" -t commands "$OBJECT" >"$commands"
  "$PYBIN" "$COMPILE_OVERLAY" --commands-log "$commands" \
    --relative "$RELATIVE" --source-root "$SOURCE_ROOT" \
    --source "$SOURCE_ROOT/$RELATIVE" --output "$output_object" \
    >"$OUT/${graph_label}-object_transform.stdout"
done
...
printf 'PASS m5-object-axis-target-depth-cache native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s canonical_sha256=%s emcc=%s node=v22.16.0 callers=1 depth_followups=1 remaining_sync=2 live_receipt=false\n' \

...
|---|---|
| `/Users/paws/blender-web` in M4, current M5 click-pick/canvas-smoke/ROI-latency, current M6 Workbench/EEVEE capture, and the current M7 USD browser producer | Removed; roots are derived from `import.meta.url` in each current producer. Retained historical rigs remain immutable. |
| `NODE_PATH=/Users/paws/plushly/game-platform/node_modules` | M4, current M5 click-pick/canvas-smoke/ROI-latency, current M6 producers, the current M7 USD browser producer, and the M8 product producer use `BW_NODE_MODULES=$PWD/.m4-node/node_modules`; Playwright 1.61.1, PNGJS 7.0.0, and host-only Sharp 0.35.3 install into that local prefix for the accepted-adapter replay. The latency producer also binds libvips 8.18.3. |
...
| `.m8-browsers/Google Chrome.app`, `Microsoft Edge.app`, signed geckodriver | Not transferred. The current M8 Chrome+Edge matrix, 30-second product producer, and pinned performance producer now verify canonical Linux package ELFs through exact `dpkg`/APT/keyring identities; the performance row also selects the Linux stable-release feed. Install instructions and paths are in `sandbox/m8-launch-gate/README.md`. Their live rows still wait on s7/APPLY. Port the remaining M8 soak/staged producers before using them on Linux; the M7 Safari row remains macOS-only. |

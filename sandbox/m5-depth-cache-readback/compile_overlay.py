...
"""Compile the reconstructed View3D source with a locked product-graph command."""
...
def main() -> int:
    ...
    original_source = Path(argv[source_index]).resolve()
    argv[source_index] = str(source)
    argv[output_index] = str(args.output.resolve())

    source_root = args.source_root.resolve()
    overlay_include_dirs = (
        source.parent,
        source_root / "source/blender/editors/include",
    )
    for include_dir in reversed(overlay_include_dirs):
        argv.insert(1, "-I" + str(include_dir))
    # Quoted headers absent from the selected replay fall back to the pinned live
    # source parent, while every selected canonical header wins through the
    # overlay include directories above.
    argv.insert(1, "-I" + str(original_source.parent))
    ...
    print(
        "M5_DEPTH_CACHE_TU_COMPILE_PASS "

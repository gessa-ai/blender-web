...
//
// Strict current-artifact M5 composite latency receipt:
//   * trusted Tab keydown -> Blender object.editmode_toggle operator start;
//   * separate trusted Z keydown -> direct Playwright screenshot visible change.
// The two probes are deliberately not conflated. Screenshot response completion
// is an honest upper bound on present. Pixel detection and 100/150/33 ms budgets
// are unchanged from notes/m5-latency-budget.md.
...
def _m5c_arm():
    ...
    if _m5c["armed"]: return None
    _m5c["armed"]=True
    for k in range(5):
        t0=time.time()
        try: bpy.ops.ed.undo_push(message="m5-composite-lat-cal-%d"%k)
        except Exception as exc: _m5c_emit("M5_COMPOSITE_CAL_ERR",k,repr(exc))
        t1=time.time()
        _m5c_emit("M5_COMPOSITE_CAL",k,"%.6f"%t0,"%.6f"%t1)
    _m5c_emit("M5_COMPOSITE_READY",json.dumps({"view3d":v},sort_keys=True,separators=(",",":")))
    return None
bpy.app.timers.register(_m5c_arm,first_interval=0.0,persistent=False)
`.trim();
...
async function signature(sharp, buffer) {
  return new Uint8Array(await sharp(buffer).greyscale().resize(160, 90, { fit: 'fill' }).raw().toBuffer());
}
...
async function capture(page, sharp) {
  const request = Date.now() / 1000;
  const response = await page.screenshot({ type: 'jpeg', quality: 70 });
  const completed = Date.now() / 1000;
  return {
    requestEpochSeconds: request, responseEpochSeconds: completed,
    responseMs: (completed - request) * 1000,
    signature: await signature(sharp, response),
  };
}
...
function runSelfcheck() {
  const parsed = parseClogStarted('00:17.220 operator | Started bpy.ops.object.editmode_toggle()');
  ...
  process.stdout.write(JSON.stringify({ status: 'PASS', checks: 3, samplesPerProbe: N, budgets: BUDGETS }, null, 2) + '\n');
}
...
async function main() {
  ...
  try {
    ...
    await page.addInitScript(({ calibrator }) => {
      ...
      window.__m5Composite = { phase: null, next: 0, inputs: [] };
      window.addEventListener('keydown', (event) => {
        const state = window.__m5Composite;
        const matches = state.phase === 'tab' ? event.key === 'Tab' && event.code === 'Tab' :
          state.phase === 'z' ? event.key === 'z' && event.code === 'KeyZ' : false;
        if (!matches) return;
        const index = state.next++;
        const epochSeconds = (performance.timeOrigin + event.timeStamp) / 1000;
        const record = {
          phase: state.phase, index, epochSeconds, key: event.key, code: event.code,
          isTrusted: event.isTrusted, targetId: event.target?.id || null,
          activeId: document.activeElement?.id || null,
        };
        state.inputs.push(record);
        console.error(`M5_COMPOSITE_DISPATCH ${state.phase} ${index} ${epochSeconds.toFixed(6)}`);
      }, true);
    ...
    const readyLine = await waitConsole(consoleLines, (line) => line.startsWith('M5_COMPOSITE_READY '),
      options.timeoutMs, 'M5_COMPOSITE_READY');
    ready = JSON.parse(readyLine.slice('M5_COMPOSITE_READY '.length));
    ...
    // Prewarm edit-mode drawing, then return to Object mode before measurements.
    const editCount = () => consoleLines.filter((line) => line.includes('| Started bpy.ops.object.editmode_toggle')).length;
    const waitEdit = async (minimum) => {
      const deadline = Date.now() + options.timeoutMs;
      while (Date.now() < deadline) {
        if (editCount() >= minimum) return;
        await sleep(20);
      }
      throw new Error(`timed out waiting for edit operator ${minimum}`);
    };
    const before = editCount();
    await page.keyboard.press('Tab');
    await waitEdit(before + 1);
    await sleep(15000);
    await page.keyboard.press('Tab');
    await waitEdit(before + 2);
    await sleep(1500);

    // Phase A: trusted Tab -> exact Blender core operator-start sub-budget.
    await page.evaluate(() => { window.__m5Composite.phase = 'tab'; window.__m5Composite.next = 0; });
    for (let index = 0; index < N; index++) {
      await page.keyboard.press('Tab');
      const input = await page.evaluate((wanted) =>
        window.__m5Composite.inputs.find((item) => item.phase === 'tab' && item.index === wanted) || null, index);
      if (!input) throw new Error(`missing Tab input ${index}`);
      tabDispatchesLive.push(input);
      await sleep(SPACING_MS);
    }
    await page.evaluate(() => { window.__m5Composite.phase = null; });
    await sleep(1000);

    // Phase B: independently calibrated direct screenshot noise + trusted Z
    // visual response. Escape resets the pie menu between samples and is not a
    // measured input.
    const noiseShots = [];
    for (let index = 0; index < 7; index++) {
      const shot = await capture(page, sharp);
    ...
    await page.evaluate(() => { window.__m5Composite.phase = 'z'; window.__m5Composite.next = 0; });
    for (let index = 0; index < N; index++) {
      const baseline = await capture(page, sharp);
      captureResponseMs.push(baseline.responseMs);
      // Keep the shading pie visibly open while polling. Keydown is the measured
      // trusted input; keyup/Escape occur only after the first changed capture.
      await page.keyboard.down('z');
      const input = await page.evaluate((wanted) =>
        window.__m5Composite.inputs.find((item) => item.phase === 'z' && item.index === wanted) || null, index);
      if (!input) throw new Error(`missing Z input ${index}`);
      ...
      zVisualLive.push({ index, dispatchEpochSeconds: input.epochSeconds, visibleEpochSeconds, maxDifference, captureCount });
    ...
    await page.evaluate(() => { window.__m5Composite.phase = null; });
    await sleep(1000);
    trustedInputs = await page.evaluate(() => window.__m5Composite.inputs);
  ...
  for (const line of consoleLines) {
    let match = /^M5_COMPOSITE_CAL (\d+) ([\d.]+) ([\d.]+)/.exec(line);
    if (match) {
      calibrations.push({ index: +match[1], start: +match[2], end: +match[3] });
      continue;
    }
    match = /^M5_COMPOSITE_DISPATCH tab (\d+) ([\d.]+)/.exec(line);
    if (match) {
      tabDispatches.push({ index: +match[1], epochSeconds: +match[2] });
      continue;
    }
  ...
  const calibrationOps = operators.filter((item) =>
    item.operator === 'bpy.ops.ed.undo_push' && item.line.includes('m5-composite-lat-cal-')).slice(0, 5);
  ...
  let operatorIndex = 0;
  for (let index = 0; index < tabDispatches.length; index++) {
    const dispatch = tabDispatches[index];
    const end = index + 1 < tabDispatches.length ? tabDispatches[index + 1].epochSeconds :
  ...
  const visualSamples = zVisualLive.map((sample) => ({
    index: sample.index,
    endToEndMs: sample.visibleEpochSeconds == null ? null :
      (sample.visibleEpochSeconds - sample.dispatchEpochSeconds) * 1000,
    maxDifference: round1(sample.maxDifference), captureCount: sample.captureCount,
    hasVisible: sample.visibleEpochSeconds != null,
  }));
  const metrics = {
    tabKeypressToOperatorMs: roundedStats(operatorSamples.map((sample) => sample.keypressToOperatorMs)),
    zEndToEndMs: roundedStats(visualSamples.map((sample) => sample.endToEndMs)),
  ...
  const tabInputs = trustedInputs.filter((item) => item.phase === 'tab');
  const zInputs = trustedInputs.filter((item) => item.phase === 'z');
  const inputPass = tabInputs.length === N && zInputs.length === N &&
    tabInputs.every((item, index) => item.index === index && item.key === 'Tab' && item.code === 'Tab' &&
      item.isTrusted && item.targetId === 'canvas' && item.activeId === 'canvas') &&
    zInputs.every((item, index) => item.index === index && item.key === 'z' && item.code === 'KeyZ' &&
      item.isTrusted && item.targetId === 'canvas' && item.activeId === 'canvas');
  const measurementPass = operatorSamples.length === N && visualSamples.length === N &&
    metrics.tabKeypressToOperatorMs.n === N && metrics.zEndToEndMs.n === N;
  const budgetPass = measurementPass &&
    metrics.tabKeypressToOperatorMs.median <= BUDGETS.keypressToOperatorMedianMs &&
    metrics.zEndToEndMs.median <= BUDGETS.endToEndMedianMs &&
    metrics.zEndToEndMs.p95 <= BUDGETS.endToEndP95Ms;
  ...
  const operatorSamplesPath = join(outDir, 'tab-operator-samples.json');
  const visualSamplesPath = join(outDir, 'z-visual-samples.json');
  ...
  writeFileSync(operatorSamplesPath, JSON.stringify(operatorSamples, null, 2) + '\n');
  writeFileSync(visualSamplesPath, JSON.stringify(visualSamples, null, 2) + '\n');
  ...
  const receipt = {
    schema: 'blender-web.m5-trusted-composite-latency.v1',
    run: options.run,
    ...
    contract: {
      ...
      operatorProbe: '10 trusted Tab keydowns -> bpy.ops.object.editmode_toggle operator start',
      visualProbe: '10 separate trusted held-Z keydowns -> direct Playwright screenshot visible response',
      separation: 'Z is visual-only and is never represented as a Tab/edit-mode operator sample',
    ...
    result: {
      ...
      nTabRequested: N, nTabDispatch: tabDispatches.length, nTabWithOperator: metrics.tabKeypressToOperatorMs.n,
      nZRequested: N, nZDispatch: zInputs.length, nZWithVisible: metrics.zEndToEndMs.n,
      ...
      metrics, operatorSamples, visualSamples, canvas: canvasReceipt, ready,
    },
    ...
    artifacts: {
      ...
      tabOperatorSamples: fileReceipt(operatorSamplesPath), zVisualSamples: fileReceipt(visualSamplesPath),

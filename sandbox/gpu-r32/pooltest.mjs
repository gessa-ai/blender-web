...
const DVIEW = (process.argv[6] || '') === 'dview';
const FIXSTENCIL = (process.argv[7] || '') === 'fixstencil';
const DCLEAR = (process.argv[8] || '') === 'dclear';
const W = 1280, H = 720;
...
const PYEXPR = [
  ...
  ...(NOCACHE ? ['os.environ["BW_NOCACHE"] = "1"'] : []),
  ...(DVIEW ? ['os.environ["BW_DVIEW"] = "1"'] : []),
  ...(FIXSTENCIL ? ['os.environ["BW_FIXSTENCIL"] = "1"'] : []),
  ...(DCLEAR ? ['os.environ["BW_DCLEAR"] = "1"'] : []),
...
const depthTargets = dones.map(parseKV).filter((o) => o.file && (o.label || '').includes('depth'));
...
try {
  analyses = await page.evaluate((targets) => {
    ...
    return targets.map((t) => {
      ...
      return { label: t.label, w, h, texel, writtenFrac: +(written / total).toFixed(4), min: dmin, max: dmax };
    });
  }, depthTargets);

...
RELATIVE="source/blender/editors/gpencil_legacy/annotate_paint.cc"
OBJECT_TARGET="source/blender/editors/gpencil_legacy/CMakeFiles/bf_editor_gpencil_legacy.dir/annotate_paint.cc.o"
...
mkdir -p "$NATIVE_BUILD" "$WASM_BUILD" "$OUT" "$ROOT/build-deps/m5-annotation-depth-cache"
if [ -z "$SOURCE_ROOT" ]; then
  SOURCE_ROOT="$(mktemp -d -t bw-m5-annotation-depth-cache-source.XXXXXX)"
  cleanup_source()
  {
    rm -rf -- "$SOURCE_ROOT"
  }
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" m5_annotation_depth_cache_contract
...
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" m5_annotation_depth_cache_contract

NATIVE_STDOUT="$OUT/native.stdout"
NATIVE_STDERR="$OUT/native.stderr"
WASM_STDOUT="$OUT/wasm.stdout"
WASM_STDERR="$OUT/wasm.stderr"
"$NATIVE_BUILD/m5_annotation_depth_cache_contract" >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_annotation_depth_cache_contract.js" >"$WASM_STDOUT" 2>"$WASM_STDERR"
...
if ! jq -e '
  .verdict == "PASS" and
  .contracts.interactive_draw_completion == true and
  .contracts.interactive_polyline_updates == true and
  .contracts.interactive_depth_eraser == true and
...
for graph in "$NATIVE_GRAPH" "$WASM_GRAPH"; do
  ...
  commands="$OUT/${graph_label}-annotate_paint.commands"
  object="$OUT/${graph_label}-annotate_paint.o"
  "$ROOT/scripts/ninja-locked.sh" -C "$graph" -t commands "$OBJECT_TARGET" >"$commands"
  "$PYBIN" "$HERE/compile_overlay.py" --commands-log "$commands" \
    --relative "$RELATIVE" --source-root "$SOURCE_ROOT" --source "$SOURCE_ROOT/$RELATIVE" \
    --output "$object" >"$OUT/${graph_label}-annotate_paint.stdout"
done

"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" -n m5_annotation_depth_cache_contract
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" -n m5_annotation_depth_cache_contract

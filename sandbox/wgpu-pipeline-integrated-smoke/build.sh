...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(wc -l <"$stdout_file" | tr -d ' ')" -ne 44 ] ||
     ...
     ! grep -qx \
       ...
       "$stdout_file" ||
     ! grep -qx \
       'CONTRACT resize_present_barrier_queue PASS cases=6 order=prior,barrier,present,release,later' \
       "$stdout_file" ||
     ! grep -qx \

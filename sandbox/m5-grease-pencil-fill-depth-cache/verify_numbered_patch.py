...
"""Reverse and forward patch 0270 against its exact current postimage."""
...
EXPECTED_NAME = "0270-m5-grease-pencil-fill-depth-cache-continuation.patch"
EXPECTED_PATHS = {
    ...
    Path("source/blender/editors/sculpt_paint/grease_pencil/fill.cc"),
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="m5-gp-fill-depth-patch-") as temporary:
    ...
    print(f"M5_GREASE_PENCIL_FILL_DEPTH_CACHE_NUMBERED_PATCH_PASS files=3 sha256={digest}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"M5_GREASE_PENCIL_FILL_DEPTH_CACHE_NUMBERED_PATCH_FAIL {error}", file=__import__("sys").stderr)
        raise SystemExit(1)

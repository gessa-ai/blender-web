...
- `probe_error_handles.cc` — an audit-only software-adapter control proving that
  ...
  next-epoch retry. The matching compute-bind-group control additionally requires its rejected and
  accepted attempts to leave the device uncaptured-error counter at zero before only the clean
  retry releases dependent work. The GHOST resize control feeds a real non-null texture error
  object through the shipping scope and resize helpers, preserving the old
  authoritative/surface/backbuffer state before a clean replacement commits every extent together.
  Matching bind-group, texture, and texture-view controls prove the same ordered

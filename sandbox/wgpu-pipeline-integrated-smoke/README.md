...
The synchronous getter allocates a caller-owned UTF-8 result from the shared Wasm allocator. Primary
selection and image clipboard remain unadvertised; diagnostics expose lengths and status only. The
source contract rejects 17 ownership, ordering, permission, selection, and privacy mutations, and
the real worker-topology browser harness covers external paste, system copy, Unicode/newlines, and
the null-versus-empty distinction.

...
| `NODE_PATH=/Users/paws/plushly/game-platform/node_modules` | M4, current M5 click-pick/canvas-smoke/ROI-latency, current M6 producers, and the current M7 USD browser producer use `BW_NODE_MODULES=$PWD/.m4-node/node_modules`; Playwright 1.61.1 and host-only Sharp 0.35.3 install into that local prefix for the accepted-adapter replay. The latency producer also binds libvips 8.18.3. |
...
| `.m8-browsers/Google Chrome.app`, `Microsoft Edge.app`, signed geckodriver | Not transferred. The current M8 Chrome+Edge matrix now verifies canonical Linux package ELFs through exact `dpkg`/APT/keyring identities; install instructions and paths are in `sandbox/m8-launch-gate/README.md`. Its live rows still wait on s7/APPLY. Port the separate M8 performance/product/soak/staged producers before using them on Linux; the M7 Safari row remains macOS-only. |

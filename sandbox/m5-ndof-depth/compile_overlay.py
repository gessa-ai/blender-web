...
"""Compile reconstructed NDOF sources with a locked product graph command."""
...
def main() -> int:
    ...
    for definition in reversed(args.define):
        argv.insert(1, "-D" + definition)
    # Quoted sibling headers must come from the pinned live source tree. The
    # focused source gate independently binds reconstructed GPU API postimages;
    # mixing their temporary paths with live GPU_viewport.hh would load the same
    # pragma-once header through two identities.

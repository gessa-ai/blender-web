...
# Canonical WebGPU bind-space and sampler-policy parity smoke
...
extracted sampler policy, and all eight direct source/header inputs before
evidence allocation. Both targets build only
through `scripts/ninja-locked.sh` and finish with exact no-work checks. The
self-check requires malformed sampler source, wrong Dawn, wrong Node, and
drifted fmt identities to fail before their requested evidence directories are
allocated.

No WebGPU instance, adapter, device, sampler, bind group, command, or receipt is

...
wgpu::BufferUsage usage_flags(BufferKind kind, UsageType usage, bool readable)
{
  ...
  switch (kind) {
    case BufferKind::Vertex:
      /* VERTEX_BUFFER | STORAGE_BUFFER (subdiv/compute writes) — vk_vertex_buffer.cc:207. */
      u |= wgpu::BufferUsage::Vertex | wgpu::BufferUsage::Storage;
      break;
    case BufferKind::Index:
      /* INDEX_BUFFER | STORAGE_BUFFER — vk_index_buffer.cc:113. */
      u |= wgpu::BufferUsage::Index | wgpu::BufferUsage::Storage;
      break;
    case BufferKind::Uniform:
      /* UNIFORM_BUFFER (+STORAGE in vk, omitted here — WebGPU forbids a buffer
       * being both Uniform and Storage bound; keep it uniform-only). vk_uniform_buffer.cc:42. */
      u |= wgpu::BufferUsage::Uniform;
      break;
    case BufferKind::Storage:
      /* STORAGE_BUFFER | INDIRECT — vk_storage_buffer.cc:82. */
...
bool Buffer::create(const wgpu::Device &device, BufferKind kind, UsageType usage,
...
bool Buffer::update_sub(const wgpu::Device &device, const wgpu::Queue &queue,
...
std::vector<uint8_t> Buffer::read(const wgpu::Instance &instance, const wgpu::Device &device,
                                  const wgpu::Queue &queue, size_t offset, size_t size)
{
  ...
  bool ok = false;
  wgpu::Future f = staging.MapAsync(wgpu::MapMode::Read, 0, copy, wgpu::CallbackMode::WaitAnyOnly,

...
"""Reverse and forward patch 0271 against its exact current postimage."""
...
EXPECTED_NAME = "0271-m5-grease-pencil-pen-depth-cache-continuation.patch"
EXPECTED_PATHS = {
    Path("source/blender/editors/include/ED_curves.hh"),
    Path("source/blender/editors/curves/intern/curves_pen.cc"),
    Path("source/blender/editors/grease_pencil/intern/grease_pencil_pen.cc"),
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="m5-gp-pen-depth-patch-") as temporary:
        ...
        if all(
            (replay / relative).read_bytes() == postimage
            for relative, postimage in postimages.items()
        ):
    ...
    print(f"M5_GREASE_PENCIL_PEN_DEPTH_CACHE_NUMBERED_PATCH_PASS files=3 sha256={digest}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"M5_GREASE_PENCIL_PEN_DEPTH_CACHE_NUMBERED_PATCH_FAIL {error}",

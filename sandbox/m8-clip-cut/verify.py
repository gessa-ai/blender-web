...
"""Fail closed on the windowed-only Clip editor registration cut."""
...
PATCH_NAME = "0253-web-windowed-clip-registration-cut.patch"
PATCH = ROOT / "patches" / PATCH_NAME

GUARD = "#if !defined(__EMSCRIPTEN__) || defined(WITH_BLENDER_WEB_CLIP)"
SPACE_CALL = "ED_spacetype_clip();"
MACRO_CALL = "ED_operatormacros_clip();"
CALLS = (SPACE_CALL, MACRO_CALL)
OPTION_RE = re.compile(
    r'option\(WITH_BLENDER_WEB_CLIP\s+'
    r'"blender-web: register non-launch Clip editor"\s+ON\)'
)
WINDOWED_OFF_RE = re.compile(
    r'set\(WITH_BLENDER_WEB_CLIP\s+OFF\s+CACHE BOOL "" FORCE\)'
)
CMAKE_FALLBACK = """if(NOT DEFINED WITH_BLENDER_WEB_CLIP)
  if(WITH_BLENDER_WEB_WINDOWED)
    set(WITH_BLENDER_WEB_CLIP OFF)
  else()
    set(WITH_BLENDER_WEB_CLIP ON)
  endif()
endif()
if(WITH_BLENDER_WEB_CLIP)
  target_compile_definitions(bf_editor_space_api PRIVATE WITH_BLENDER_WEB_CLIP)
endif()"""
SHIPPING_PATHS = (
...
def verify(config: str, cmake: str, source: str, series: str, patch: str) -> list[str]:
    ...
    if len(OPTION_RE.findall(config)) != 1:
        errors.append("config must declare the Clip option exactly once with default ON")
    if len(WINDOWED_OFF_RE.findall(config)) != 1:
        errors.append("windowed profile must force the Clip option OFF exactly once")
    if CMAKE_FALLBACK not in cmake:
        errors.append("space_api CMake fallback/compile-definition contract changed")
    if cmake.count("bf_editor_space_clip") != 1:
        errors.append("Clip editor library must remain linked exactly once")

    totals, guarded, source_errors = guarded_call_counts(source)
    errors.extend(source_errors)
    for call in CALLS:
        if totals[call] != 1:
            errors.append(f"Clip registration count changed for {call}: {totals[call]}")
        if guarded[call] != 1:
            errors.append(f"Clip registration escaped the web feature guard: {call}")
    ...
    for fragment in forbidden_fragments:
        if fragment in patch:
            errors.append(f"numbered patch crosses the retained Clip data/editor boundary: {fragment}")
    return errors
...
def run_selfcheck(inputs: tuple[str, str, str, str, str]) -> int:
    ...
    controls = (
        (config.replace('Clip editor" ON)', 'Clip editor" OFF)', 1), cmake, source, series, patch),
        (
            config.replace("WEB_CLIP OFF CACHE", "WEB_CLIP ON CACHE", 1),
            cmake,
            source,
            series,
            patch,
        ...
        (config, cmake.replace("  bf_editor_space_clip\n", "", 1), source, series, patch),
    )
...
def run_roundtrip() -> str:
    with tempfile.TemporaryDirectory(prefix="m8-clip-cut-") as temp_name:
...
def verify_build_rules() -> int:
    definition = "-DWITH_BLENDER_WEB_CLIP"
    pattern = re.compile(
    ...
    for path, expect_definition in BUILD_RULES:
        ...
        if present != expect_definition:
            raise RuntimeError(
                f"Clip definition mismatch in {path.relative_to(ROOT)}: "
...
def main() -> int:
    ...
    try:
        ...
        print(f"M8_CLIP_CUT_FAIL {error}")
        return 1
    errors = verify(*inputs)
    if errors:
        print("M8_CLIP_CUT_FAIL " + " | ".join(errors))
        return 1
    try:

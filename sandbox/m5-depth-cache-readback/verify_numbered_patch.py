...
"""Reverse and forward patch 0264 against its exact current postimage."""
...
EXPECTED_NAME = "0264-m5-depth-cache-readback-primitive.patch"
EXPECTED_PATHS = {
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="m5-depth-cache-patch-") as temporary:
    ...
    print(f"M5_DEPTH_CACHE_NUMBERED_PATCH_PASS files=2 sha256={digest}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"M5_DEPTH_CACHE_NUMBERED_PATCH_FAIL {error}", file=__import__("sys").stderr)
        raise SystemExit(1)

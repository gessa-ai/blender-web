...
NATIVE_BUILD="${NATIVE_BUILD:-$ROOT/build-deps/m5-curve-depth-cache/native}"
WASM_BUILD="${WASM_BUILD:-$ROOT/build-deps/m5-curve-depth-cache/wasm}"
OUT="${OUT:-$ROOT/build-deps/m5-curve-depth-cache/evidence}"
PATCH="$ROOT/patches/0265-m5-curve-depth-cache-continuation.patch"
...
SOURCE_SPECS=(
  "source/blender/editors/space_view3d/view3d_draw.cc:source/blender/editors/space_view3d/CMakeFiles/bf_editor_space_view3d.dir/view3d_draw.cc.o"
  "source/blender/editors/curve/editcurve_paint.cc:source/blender/editors/curve/CMakeFiles/bf_editor_curve.dir/editcurve_paint.cc.o"
  "source/blender/editors/curves/intern/curves_draw.cc:source/blender/editors/curves/CMakeFiles/bf_editor_curves.dir/intern/curves_draw.cc.o"
)
...
mkdir -p "$NATIVE_BUILD" "$WASM_BUILD" "$OUT" "$ROOT/build-deps/m5-curve-depth-cache"
if [ -z "$SOURCE_ROOT" ]; then
  SOURCE_ROOT="$(mktemp -d -t bw-m5-curve-depth-cache-source.XXXXXX)"
  cleanup_source()
  {
    rm -rf -- "$SOURCE_ROOT"
  }
  ...
  git -C "$ROOT/upstream" archive --format=tar "$PIN" -- \
    ...
    source/blender/editors/curves/intern/curves_draw.cc | tar -xf - -C "$SOURCE_ROOT"
  git -C "$SOURCE_ROOT" apply --check --include='source/blender/editors/include/**' "$CANONICAL"
  git -C "$SOURCE_ROOT" apply --include='source/blender/editors/include/**' "$CANONICAL"
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" m5_curve_depth_cache_contract
...
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" m5_curve_depth_cache_contract

NATIVE_STDOUT="$OUT/native.stdout"
NATIVE_STDERR="$OUT/native.stderr"
WASM_STDOUT="$OUT/wasm.stdout"
WASM_STDERR="$OUT/wasm.stderr"
"$NATIVE_BUILD/m5_curve_depth_cache_contract" >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_curve_depth_cache_contract.js" >"$WASM_STDOUT" 2>"$WASM_STDERR"
...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(grep -c '^CONTRACT .* PASS ' "$stdout_file")" -ne 8 ] ||
     ! grep -qx 'M5_CURVE_DEPTH_CACHE_CONTRACT_PASS contracts=8 cases=16' "$stdout_file"; then
...
if ! jq -e '
  .verdict == "PASS" and
  .contracts.two_curve_draw_lanes == true and
  ...
  .converted_callers == ["CURVE_OT_draw", "CURVES_OT_draw"] and
...
for graph in "$NATIVE_GRAPH" "$WASM_GRAPH"; do
  ...
  for spec in "${SOURCE_SPECS[@]}"; do
    ...
    "$PYBIN" "$HERE/compile_overlay.py" --commands-log "$commands" \
      --relative "$relative" --source-root "$SOURCE_ROOT" --source "$SOURCE_ROOT/$relative" \
      --output "$object" >"$OUT/${graph_label}-${stem}.stdout"
  done
done

"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" -n m5_curve_depth_cache_contract
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" -n m5_curve_depth_cache_contract
...
printf 'PASS m5-curve-depth-cache native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s canonical_sha256=%s emcc=%s node=v22.16.0 callers=2 remaining_sync=2 live_receipt=false\n' \

...
FIRST_PIXEL_VERDICT='CONTRACT ghost_redraw_recovery PASS cases=48 periodic=15 late=immediate drops=bounded readiness=rearmed resize_commit=fresh present_barrier=ordered-sync-commit-superseded trace=bounded-exact wrap=rearmed'
...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(wc -l <"$stdout_file" | tr -d ' ')" -ne 44 ] ||
     ...
     ! grep -qx \
       ...
       "$stdout_file" ||
     ! grep -qx \
       'CONTRACT resize_present_barrier_queue PASS cases=31 frame_binding=3 order=prior,barrier,present,release,later recovery=failed-frame,failed-present,retry supersession=queued,ready,stale-completion' \
       "$stdout_file" ||
     ! grep -qx \

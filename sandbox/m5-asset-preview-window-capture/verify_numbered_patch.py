...
"""Reverse and reapply patch 0274 against its exact canonical postimage."""
...
RELATIVE = Path("source/blender/editors/asset/intern/asset_ops.cc")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bw-m5-asset-preview-patch-") as temporary:
        root = Path(temporary)
        ...
        if predecessor.count("WM_window_pixels_read(C, win, dumprect_size)") != 1:
            raise RuntimeError("reversed predecessor does not restore synchronous asset capture")
        if "WM_window_pixels_read_async(C, win)" in predecessor:
            raise RuntimeError("reversed predecessor retains asset async capture")
    ...
    print(
        "M5_ASSET_PREVIEW_WINDOW_CAPTURE_PATCH_PASS "
        f"paths=1 reverse_sync=1 sha256={digest}"
    )
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"M5_ASSET_PREVIEW_WINDOW_CAPTURE_PATCH_FAIL {error}")
        raise SystemExit(1)

...
SPARSE_HARDWARE_SERIES_CONTRACT="$ROOT/sandbox/p0-interaction-stress/analyze_sparse_hardware_series.py"
...
require_file "$SPARSE_HARDWARE_SERIES_CONTRACT"
...
require_fixed_count 1 \
  'ghost_web::input_redraw_trace_frame_begin(ghost_web::input_redraw_dispatched_count(),' \
...
"$PYBIN" "$SPARSE_HARDWARE_SERIES_CONTRACT" --self-check
...
FIRST_PIXEL_VERDICT='CONTRACT ghost_redraw_recovery PASS cases=104 periodic=15 late=immediate drops=bounded readiness=rearmed input=bounded-burst+content-retired-terminal-tail resize_commit=fresh present_settlement=coalesced-wm-retry present_telemetry=suppressed-wm-replayed input_delivery=callback+wm-queue-balanced-mask input_presentation=monotonic-frame-bound+view3d-content present_barrier=ordered-sync-commit-superseded trace=bounded-exact viewport_ready=grid-validated-one-shot wrap=rearmed'

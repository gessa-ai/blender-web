...
def validate(capture: str, analyzer: str) -> None:
    for marker in (
        ...
        'await capture("70a-modal-extrude");',
        'await capture("70b-modal-settle-500ms");',
        'await capture("70c-modal-settle-3s");',
        'await capture("70d-modal-settle-6s");',
    ...
    for marker in (
        ...
        'contract.get("modalArtifacts") == {',
        'modal.get("operations") == ["E Z confirm", "G X cancel", "R Z cancel", "S X cancel"]',
        'settle_3_to_6s.get("viewChangedFraction")',
        'bars["maxConsecutiveWideRows"] <= MODAL_MAX_WIDE_NEUTRAL_ROWS',

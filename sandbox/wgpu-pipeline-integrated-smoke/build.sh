...
"$PYBIN" "$WINDOW_LIFECYCLE_CONTRACT" \
  "$GHOST_SYSTEM_HEADER" "$GHOST_SYSTEM_SOURCE" --selfcheck
...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(wc -l <"$stdout_file" | tr -d ' ')" -ne 41 ] ||

...
NATIVE_BUILD="${NATIVE_BUILD:-$ROOT/build-deps/m5-depth-eyedropper/native}"
WASM_BUILD="${WASM_BUILD:-$ROOT/build-deps/m5-depth-eyedropper/wasm}"
OUT="${OUT:-$ROOT/build-deps/m5-depth-eyedropper/evidence}"
PATCH="$ROOT/patches/0258-m5-depth-eyedropper-continuation.patch"
NATIVE_GRAPH="$ROOT/build-native-m1-parity"
WASM_GRAPH="$ROOT/build-wasm-windowed-opt"
OBJECT_TARGET="source/blender/editors/interface/CMakeFiles/bf_editor_interface.dir/eyedroppers/eyedropper_depth.cc.o"
...
for file in "$HOST_CMAKE" "$PYBIN" "$NODE" "$ROOT/scripts/ninja-locked.sh" \
            ...
            "$HERE/verify_numbered_patch.py" "$HERE/compile_overlay.py" "$PATCH"; do
  require_file "$file"
done
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" m5_depth_eyedropper_contract
...
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" m5_depth_eyedropper_contract

NATIVE_STDOUT="$OUT/native.stdout"
NATIVE_STDERR="$OUT/native.stderr"
WASM_STDOUT="$OUT/wasm.stdout"
WASM_STDERR="$OUT/wasm.stderr"
"$NATIVE_BUILD/m5_depth_eyedropper_contract" >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_depth_eyedropper_contract.js" >"$WASM_STDOUT" 2>"$WASM_STDERR"
...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(grep -c '^CONTRACT .* PASS ' "$stdout_file")" -ne 6 ] ||
     ! grep -qx 'M5_DEPTH_EYEDROPPER_CONTRACT_PASS contracts=6 cases=13' "$stdout_file"; then
...
if ! jq -e '
  ...
  .contracts.depth_eyedropper_modal_continuation == true and
  .contracts.producing_context_guard == true and
  .contracts.exact_view_aligned_depth == true and
  .contracts.accumulate_and_reset_replay == true and
  .contracts.confirm_waits_for_inflight_sample == true and
  .contracts.latest_drag_sample_supersedes_pending == true and
  .contracts.bounded_failure_cancellation == true and
  .contracts.pinned_direct_confirm_behavior == true and
  .contracts.live_hardware_receipt == false and
  .converted_consumer == "depth_eyedropper" and
  .remaining_depth_pick_consumers == ["navigation", "painting", "zoom_border", "ndof"] and
...
"$PYBIN" "$HERE/compile_overlay.py" --commands-log "$OUT/native.commands" \
  --source "$SOURCE_ROOT/source/blender/editors/interface/eyedroppers/eyedropper_depth.cc" \
  --output "$NATIVE_BUILD/eyedropper_depth.overlay.o" >"$OUT/native-tu.stdout"
"$PYBIN" "$HERE/compile_overlay.py" --commands-log "$OUT/wasm.commands" \
  --source "$SOURCE_ROOT/source/blender/editors/interface/eyedroppers/eyedropper_depth.cc" \
  --output "$WASM_BUILD/eyedropper_depth.overlay.o" >"$OUT/wasm-tu.stdout"

"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" -n m5_depth_eyedropper_contract
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" -n m5_depth_eyedropper_contract
...
printf 'PASS m5-depth-eyedropper native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s emcc=%s node=v22.16.0 remaining_sync=3 live_receipt=false\n' \

...
NATIVE_BUILD="${NATIVE_BUILD:-$ROOT/build-deps/m5-zoom-border-depth/native-contract}"
...
SOURCE_REL="source/blender/editors/space_view3d/view3d_navigate_zoom_border.cc"
SOURCE_TARGET="source/blender/editors/space_view3d/CMakeFiles/bf_editor_space_view3d.dir/view3d_navigate_zoom_border.cc.o"
...
mkdir -p "$NATIVE_BUILD" "$WASM_BUILD" "$OUT" "$ROOT/build-deps/m5-zoom-border-depth"
SOURCE_ROOT="$(mktemp -d -t bw-m5-zoom-border-source.XXXXXX)"
cleanup_source()
{
  rm -rf -- "$SOURCE_ROOT"
}
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" m5_zoom_border_depth_contract
...
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" m5_zoom_border_depth_contract

NATIVE_STDOUT="$OUT/native.stdout"
NATIVE_STDERR="$OUT/native.stderr"
WASM_STDOUT="$OUT/wasm.stdout"
WASM_STDERR="$OUT/wasm.stderr"
"$NATIVE_BUILD/m5_zoom_border_depth_contract" >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_zoom_border_depth_contract.js" >"$WASM_STDOUT" 2>"$WASM_STDERR"
...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(grep -c '^CONTRACT .* PASS ' "$stdout_file")" -ne 8 ] ||
     ! grep -qx 'M5_ZOOM_BORDER_DEPTH_CONTRACT_PASS contracts=8 cases=20' "$stdout_file"; then
...
if ! jq -e '
  ...
  .contracts.stock_post_clamp_rectangle == true and
  .contracts.strict_nearest_depth == true and
  .contracts.native_immediate_completion == true and
  .contracts.gesture_to_readback_handoff == true and
  .contracts.exact_zoom_mode_smooth_replay == true and
  .contracts.perspective_and_ortho_semantics == true and
  .contracts.producing_view_drift_rejected == true and
  .contracts.newest_request_supersedes == true and
  ...
  .converted_consumer == "zoom_border" and
  .remaining_depth_pick_consumers == ["ndof"] and
...
"$PYBIN" "$HERE/compile_overlay.py" --commands-log "$OUT/native.commands" \
  ...
  --output "$NATIVE_BUILD/view3d_navigate_zoom_border.overlay.o" >"$OUT/native-tu.stdout"
"$PYBIN" "$HERE/compile_overlay.py" --commands-log "$OUT/wasm.commands" \
  --relative "$SOURCE_REL" --source-root "$SOURCE_ROOT" --source "$SOURCE_ROOT/$SOURCE_REL" \
  --output "$WASM_BUILD/view3d_navigate_zoom_border.overlay.o" >"$OUT/wasm-tu.stdout"

"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" -n m5_zoom_border_depth_contract
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" -n m5_zoom_border_depth_contract
...
printf 'PASS m5-zoom-border-depth native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s canonical_sha256=%s emcc=%s node=v22.16.0 remaining_depth=ndof remaining_sync=3 live_receipt=false\n' \

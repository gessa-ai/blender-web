...
- [ ] M0.8 Ops: disk audit (claude -p re-verified WORKING 2026-08-03; workers = sonnet via claude + codex; driver = opus) (need >=40GB free before M2 — currently ~19GB: identify reclaim or external volume; PAGE if unresolvable); write notes/codex-cli.md from `codex exec --help` (verify non-interactive + model flags for worker use).

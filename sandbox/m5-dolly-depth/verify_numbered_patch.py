...
"""Reverse and forward patch 0260 against its exact current postimage."""
...
EXPECTED_NAME = "0260-m5-dolly-depth-continuation.patch"
EXPECTED_PATHS = {
    Path("source/blender/editors/space_view3d/view3d_navigate_view_dolly.cc"),
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="m5-dolly-patch-") as temporary:
    ...
    print(f"M5_DOLLY_NUMBERED_PATCH_PASS files=1 sha256={digest}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"M5_DOLLY_NUMBERED_PATCH_FAIL {error}", file=__import__("sys").stderr)
        raise SystemExit(1)

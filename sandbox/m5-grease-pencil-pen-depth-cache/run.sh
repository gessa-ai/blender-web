...
NATIVE_BUILD="${NATIVE_BUILD:-$ROOT/build-deps/m5-grease-pencil-pen-depth-cache/native}"
WASM_BUILD="${WASM_BUILD:-$ROOT/build-deps/m5-grease-pencil-pen-depth-cache/wasm}"
OUT="${OUT:-$ROOT/build-deps/m5-grease-pencil-pen-depth-cache/evidence}"
PATCH="$ROOT/patches/0271-m5-grease-pencil-pen-depth-cache-continuation.patch"
...
OBJECTS=(
  "source/blender/editors/curves/CMakeFiles/bf_editor_curves.dir/intern/curves_pen.cc.o"
  "source/blender/editors/grease_pencil/CMakeFiles/bf_editor_grease_pencil.dir/intern/grease_pencil_pen.cc.o"
)
LABELS=("curves_pen" "grease_pencil_pen")
...
if [ -z "$SOURCE_ROOT" ]; then
  SOURCE_ROOT="$(mktemp -d -t bw-m5-gp-pen-depth-source.XXXXXX)"
  cleanup_source()
  {
    rm -rf -- "$SOURCE_ROOT"
  }
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" \
  m5_grease_pencil_pen_depth_cache_contract
...
"$NATIVE_BUILD/m5_grease_pencil_pen_depth_cache_contract" \
  >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_grease_pencil_pen_depth_cache_contract.js" \
...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(grep -c '^CONTRACT .* PASS ' "$stdout_file")" -ne 9 ] ||
     ! grep -qx \
       'M5_GREASE_PENCIL_PEN_DEPTH_CACHE_CONTRACT_PASS contracts=9 cases=33' \
       "$stdout_file"; then
...
if ! jq -e '
  ...
  .contracts.pre_mutation_suspension == true and
  .contracts.shared_post_initialize_seam == true and
  .contracts.retained_first_event_and_fifo == true and
  .contracts.stock_layer_transform_tail == true and
  ...
  .converted_callers == ["pen-helper"] and
  .remaining_grease_pencil_callers == [] and
  .remaining_sync_families == ["depth_cache", "window_capture"] and
  .mutation_controls == 14' "$OUT/source.json" >/dev/null
then
...
printf 'PASS m5-grease-pencil-pen-depth-cache native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s canonical_sha256=%s emcc=%s node=v22.16.0 callers=1 gp_followups=0 remaining_sync=2 live_receipt=false\n' \

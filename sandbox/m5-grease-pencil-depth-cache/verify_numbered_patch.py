...
"""Reverse and forward patch 0268 against its exact current postimages."""
...
EXPECTED_NAME = "0268-m5-grease-pencil-depth-cache-continuation.patch"
EXPECTED_PATHS = {
    Path("source/blender/editors/include/ED_grease_pencil.hh"),
    Path("source/blender/editors/grease_pencil/intern/grease_pencil_utils.cc"),
    Path("source/blender/editors/sculpt_paint/grease_pencil/draw_ops.cc"),
    Path("source/blender/editors/sculpt_paint/grease_pencil/grease_pencil_intern.hh"),
    Path("source/blender/editors/sculpt_paint/grease_pencil/paint.cc"),
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="m5-grease-pencil-depth-cache-patch-") as temporary:
        ...
        if all((replay / relative).read_bytes() == postimage for relative in postimages):
    ...
    print(f"M5_GREASE_PENCIL_DEPTH_CACHE_NUMBERED_PATCH_PASS files=5 sha256={digest}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"M5_GREASE_PENCIL_DEPTH_CACHE_NUMBERED_PATCH_FAIL {error}",

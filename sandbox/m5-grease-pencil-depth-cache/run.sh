...
NATIVE_BUILD="${NATIVE_BUILD:-$ROOT/build-deps/m5-grease-pencil-depth-cache/native}"
WASM_BUILD="${WASM_BUILD:-$ROOT/build-deps/m5-grease-pencil-depth-cache/wasm}"
OUT="${OUT:-$ROOT/build-deps/m5-grease-pencil-depth-cache/evidence}"
PATCH="$ROOT/patches/0268-m5-grease-pencil-depth-cache-continuation.patch"
...
SOURCE_SPECS=(
  "source/blender/editors/grease_pencil/intern/grease_pencil_utils.cc:source/blender/editors/grease_pencil/CMakeFiles/bf_editor_grease_pencil.dir/intern/grease_pencil_utils.cc.o"
  "source/blender/editors/sculpt_paint/grease_pencil/draw_ops.cc:source/blender/editors/sculpt_paint/CMakeFiles/bf_editor_sculpt_paint.dir/grease_pencil/draw_ops.cc.o"
  "source/blender/editors/sculpt_paint/grease_pencil/paint.cc:source/blender/editors/sculpt_paint/CMakeFiles/bf_editor_sculpt_paint.dir/grease_pencil/paint.cc.o"
)
...
mkdir -p "$NATIVE_BUILD" "$WASM_BUILD" "$OUT" \
  "$ROOT/build-deps/m5-grease-pencil-depth-cache"
if [ -z "$SOURCE_ROOT" ]; then
  SOURCE_ROOT="$(mktemp -d -t bw-m5-grease-pencil-depth-cache-source.XXXXXX)"
  cleanup_source()
  {
    rm -rf -- "$SOURCE_ROOT"
  }
  ...
  git -C "$ROOT/upstream" archive --format=tar "$PIN" -- \
    ...
    source/blender/editors/sculpt_paint/grease_pencil/paint.cc | tar -xf - -C "$SOURCE_ROOT"
  git -C "$SOURCE_ROOT" apply --check \
    --include='source/blender/editors/include/**' \
    --include='source/blender/editors/grease_pencil/intern/grease_pencil_utils.cc' \
    --include='source/blender/editors/sculpt_paint/grease_pencil/draw_ops.cc' \
    --include='source/blender/editors/sculpt_paint/grease_pencil/grease_pencil_intern.hh' \
    --include='source/blender/editors/sculpt_paint/grease_pencil/paint.cc' "$CANONICAL"
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" \
  m5_grease_pencil_depth_cache_contract
...
"$NATIVE_BUILD/m5_grease_pencil_depth_cache_contract" >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_grease_pencil_depth_cache_contract.js" \
...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(grep -c '^CONTRACT .* PASS ' "$stdout_file")" -ne 8 ] ||
     ! grep -qx 'M5_GREASE_PENCIL_DEPTH_CACHE_CONTRACT_PASS contracts=8 cases=18' \
       "$stdout_file"; then
...
if ! jq -e '
  ...
  .contracts.pending_first_sample == true and
  .contracts.pending_sample_fifo == true and
  .contracts.bounded_timer == true and
  .contracts.producing_context_guard == true and
  .contracts.release_after_settlement == true and
  .contracts.failure_timeout_cancellation == true and
  .contracts.live_hardware_receipt == false and
  .converted_callers == ["GREASE_PENCIL_OT_brush_stroke freehand paint"] and
  .remaining_grease_pencil_callers == ["primitive", "fill", "pen-helper"] and
...
for graph in "$NATIVE_GRAPH" "$WASM_GRAPH"; do
  ...
  for spec in "${SOURCE_SPECS[@]}"; do
    relative="${spec%%:*}"
    target="${spec#*:}"
    stem="$(basename "$relative" .cc)"
    commands="$OUT/${graph_label}-${stem}.commands"
    object="$OUT/${graph_label}-${stem}.o"
    "$ROOT/scripts/ninja-locked.sh" -C "$graph" -t commands "$target" >"$commands"
    "$PYBIN" "$COMPILE_OVERLAY" --commands-log "$commands" \
      --relative "$relative" --source-root "$SOURCE_ROOT" --source "$SOURCE_ROOT/$relative" \
      --output "$object" >"$OUT/${graph_label}-${stem}.stdout"
  done
done
...
printf 'PASS m5-grease-pencil-depth-cache native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s canonical_sha256=%s emcc=%s node=v22.16.0 callers=1 gp_followups=3 remaining_sync=2 live_receipt=false\n' \

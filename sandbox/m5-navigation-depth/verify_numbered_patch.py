...
"""Reverse and forward patch 0259 against its exact current postimage."""
...
EXPECTED_NAME = "0259-m5-navigation-depth-continuation.patch"
EXPECTED_PATHS = {
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="m5-navigation-patch-") as temporary:
    ...
    print(f"M5_NAVIGATION_NUMBERED_PATCH_PASS files=2 sha256={digest}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"M5_NAVIGATION_NUMBERED_PATCH_FAIL {error}", file=__import__("sys").stderr)
        raise SystemExit(1)

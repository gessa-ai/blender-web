...
NATIVE_BUILD="${NATIVE_BUILD:-$ROOT/build-deps/m5-navigation-depth/native}"
WASM_BUILD="${WASM_BUILD:-$ROOT/build-deps/m5-navigation-depth/wasm}"
OUT="${OUT:-$ROOT/build-deps/m5-navigation-depth/evidence}"
PATCH="$ROOT/patches/0259-m5-navigation-depth-continuation.patch"
NATIVE_GRAPH="$ROOT/build-native-m1-parity"
WASM_GRAPH="$ROOT/build-wasm-windowed-opt"
OBJECT_TARGET="source/blender/editors/space_view3d/CMakeFiles/bf_editor_space_view3d.dir/view3d_navigate.cc.o"
...
"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" m5_navigation_depth_contract
...
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" m5_navigation_depth_contract

NATIVE_STDOUT="$OUT/native.stdout"
NATIVE_STDERR="$OUT/native.stderr"
WASM_STDOUT="$OUT/wasm.stdout"
WASM_STDERR="$OUT/wasm.stderr"
"$NATIVE_BUILD/m5_navigation_depth_contract" >"$NATIVE_STDOUT" 2>"$NATIVE_STDERR"
"$NODE" "$WASM_BUILD/m5_navigation_depth_contract.js" >"$WASM_STDOUT" 2>"$WASM_STDERR"
...
for stdout_file in "$NATIVE_STDOUT" "$WASM_STDOUT"; do
  if [ "$(grep -c '^CONTRACT .* PASS ' "$stdout_file")" -ne 6 ] ||
     ! grep -qx 'M5_NAVIGATION_DEPTH_CONTRACT_PASS contracts=6 cases=9' "$stdout_file"; then
...
if ! jq -e '
  ...
  .contracts.ordinary_navigation_modal_continuation == true and
  .contracts.stock_initialization_order_once == true and
  .contracts.exact_hit_and_fallback_pivot == true and
  .contracts.latest_event_replay == true and
  .contracts.producing_context_guard == true and
  .contracts.bounded_failure_cancellation == true and
  .contracts.direct_callers_remain_pinned == true and
  .contracts.live_hardware_receipt == false and
  .converted_consumer == "ordinary_navigation" and
  .remaining_depth_pick_consumers == ["dolly", "painting", "zoom_border", "ndof"] and
...
"$PYBIN" "$HERE/compile_overlay.py" --commands-log "$OUT/native.commands" \
  --source "$SOURCE_ROOT/source/blender/editors/space_view3d/view3d_navigate.cc" \
  --output "$NATIVE_BUILD/view3d_navigate.overlay.o" >"$OUT/native-tu.stdout"
"$PYBIN" "$HERE/compile_overlay.py" --commands-log "$OUT/wasm.commands" \
  --source "$SOURCE_ROOT/source/blender/editors/space_view3d/view3d_navigate.cc" \
  --output "$WASM_BUILD/view3d_navigate.overlay.o" >"$OUT/wasm-tu.stdout"

"$ROOT/scripts/ninja-locked.sh" -C "$NATIVE_BUILD" -n m5_navigation_depth_contract
"$ROOT/scripts/ninja-locked.sh" -C "$WASM_BUILD" -n m5_navigation_depth_contract
...
printf 'PASS m5-navigation-depth native/wasm bytes=%s sha256=%s source_sha256=%s patch_sha256=%s emcc=%s node=v22.16.0 remaining_sync=3 live_receipt=false\n' \
